...
cthd_pid::cthd_pid(): kp(1.0), ki(0), kd(0), calibrated(0), last_time(0){
...
void cthd_pid::calibrate()
{
	...
	ret = pthread_create(&crazy_running_thread, &crazy_thread_attr, crazy_thread, 
	(void*)this);
	if(ret < 0)
	{
...
double cthd_pid::get_pid_output(double set_point, double xk, int state)
{
	...
	if(state &0x1)
	{
		...
		p_param.pd =  - (p_param.kp *p_param.kd / p_param.ts) *(lpf - 2.0 * lpf_1 +
	lpf_2);
...
void cthd_pid::init_pid_controller()
{
	...
	p_param.lpf1 = (2.0 *p_param.k_lpf - p_param.ts) / (2.0 *p_param.k_lpf +
	p_param.ts);
