...
// contains od -x output of sysfs data vault
#define DATA_VAULT "/tmp/data_vault"
...
size_t cthd_gddv::gddv_load(char **buffer)
{
	...
	FILE *fp = fopen (DATA_VAULT, "r");
	if (!fp)
		return 0;

	thd_log_info ("Found /tmp/data_vault\n");

	data_buffer = new char[1024 * 4];
	if (!data_buffer) {
		return 0;
	}
