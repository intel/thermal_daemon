...
int cthd_topology::calibrate()
{
	...
	for(i=0; i<no_cpu; ++i) {
		...
		for(j=0; j<no_sensors; ++j) {
			if ( i!=j && cpu_sensor_rel[j].sensor_id != -1 && cpu_sensor_rel[i].sensor_id == cpu_sensor_rel[j].sensor_id) {
				count++;
				mask |= ((1<<i) | (1<<j));
	...
	configuration_saved();
	return 0; // calibration process is done, it is possible that we can't do per cpu control
}
