...
cthd_model::cthd_model()
	: trend_increase_start(0),
	  ...
	  begin_moving_average(0),
	  current_moving_average(0),
	  trend_increase(0),
	  max_temp_reached(0),
	  current_angle(def_low_angle_from_max),
...
unsigned int cthd_model::update_set_point()
{
	...
	double delta_y = current_moving_average -  begin_moving_average;
...
void cthd_model::add_sample(unsigned int temperature)
{
	...
	current_moving_average = moving_average(temperature);
	...
	if (temperature >= max_temp) {
		...
		if (delay_cnt > def_setpoint_delay_cnt)
			update_set_point();
		delay_cnt++;
	...
	if(last_moving_average && (current_moving_average - last_moving_average) > trend_increase_offset) {
		if (!trend_increase) {
			trend_increase = 1;
			begin_moving_average = current_moving_average;
			trend_show = current_moving_average;
			trend_increase_start =  tm;
		}
	}

	if(last_moving_average && (last_moving_average - current_moving_average) > trend_increase_offset) {
		if (trend_increase) {
			trend_increase = 0;
			trend_show = current_moving_average;
			begin_moving_average = current_moving_average;
	...
	thd_log_debug("sample, Moving average, %u, %u, %d\n", temperature, current_moving_average, trend_show);
}
